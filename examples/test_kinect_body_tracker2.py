...
if __name__ == "__main__":
    ...
    for ii in range(num_source):
        ...
        locals()[f"device_{ii}"] = pykinect.start_device(config=device_config, device_index=1)
        print("Kinect_" + format(ii, "02") + " Ready")

    for ii in range(num_source):
        # Start body tracker
        locals()[f"bodyTracker_{ii}"] = pykinect.start_body_tracker(model_type=1, index=1)
        #model_type=pykinect.K4ABT_DEFAULT_MODEL
    k = 0
    ...
    while True:
        ...
        if vis:
            ...
            for ii in range(num_source):
                ...
                cv2.imshow('Depth image with skeleton_2' + str(ii), locals()[f"combined_image_{ii}"])
            #Press q key to stop
